...
@section('main')
    <main>
        ...
        <div id="tittle-list-id">
            ...
            <form action="/product-manage/parts/update/{{ $product->id }}" method="post" enctype="multipart/form-data">
                @csrf
                            ...
                            <div class="image-gradation">
                                <select name="weights" id="">
                                    <option value="1" @if ($product->weights == 1) selected @endif
                                        @foreach ($components2 as $cm2) @if ($cm2->weights == 1)
                                        hidden @endif
                                        @endforeach
                                        >1</option>
                                    <option value="2" @if ($product->weights == 2) selected @endif
                                        @foreach ($components2 as $cm2) @if ($cm2->weights == 2)
                                        hidden @endif
                                        @endforeach
                                        >2</option>
...
@section('js')
    ...
    <script>
        function d_sec_img(id) {
            ...
            fetch('/product-manage/parts/d_sec_img/' + id, {
                    method: 'POST',
                    body: formData
                })
    ...
    <script>
        function c_sec_img(id) {
            ...
            fetch('/product-manage/parts/c_sec_img/' + id, {
                method: 'POST',
                body: formData
            })
